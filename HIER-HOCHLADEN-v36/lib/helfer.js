...
/**
 * Ampel fuer einen Termin.
 *
 * Vier Zustaende, und jeder traegt ein Wort — nicht nur eine Farbe. Wer
 * Rot und Gruen nicht unterscheiden kann (etwa jeder zwoelfte Mann), liest
 * sonst nichts. Die Farbe bestaetigt die Aussage, sie ist nicht die Aussage.
 *
 * Nur "ueberfaellig" wird aus dem Datum abgeleitet: vorbei und immer noch
 * nicht als durchgefuehrt vermerkt. Ob ein vergangener Termin stattgefunden
 * hat oder ausgefallen ist, weiss die Anwendung nicht — das traegt ein
 * Mensch ein. Ein Termin still auf "durchgefuehrt" zu setzen, nur weil sein
 * Datum vorbei ist, waere eine Behauptung.
 */
function terminAmpel(t, heute = new Date()) {
  const status = String(t && t.status ? t.status : 'geplant').toLowerCase();

  if (status === 'abgesagt') {
    return { stufe: 'abgesagt', wort: 'abgesagt', klasse: 'ampel-aus' };
  }
  if (status === 'durchgeführt' || status === 'durchgefuehrt' || t.durchgefuehrt_am) {
    return { stufe: 'durchgefuehrt', wort: 'durchgeführt', klasse: 'ampel-gut' };
  }
  if (status === 'verschoben') {
    return { stufe: 'verschoben', wort: 'verschoben', klasse: 'ampel-warn' };
  }

  if (t && t.datum) {
    const tag = new Date(t.datum);
    const heuteTag = new Date(heute.getFullYear(), heute.getMonth(), heute.getDate());
    const terminTag = new Date(tag.getFullYear(), tag.getMonth(), tag.getDate());
    if (terminTag < heuteTag) {
      return { stufe: 'ueberfaellig', wort: 'nachfassen', klasse: 'ampel-spaet' };
    }
    if (terminTag.getTime() === heuteTag.getTime()) {
      return { stufe: 'heute', wort: 'heute', klasse: 'ampel-jetzt' };
    }
  }
  return { stufe: 'geplant', wort: 'geplant', klasse: 'ampel-offen' };
}
