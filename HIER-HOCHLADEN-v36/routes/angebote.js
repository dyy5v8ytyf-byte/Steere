...
// ===================== LÖSCHEN UND ZURÜCKZIEHEN =====================
/**
 * Zwei verschiedene Vorgänge hinter einem Knopf — und der Unterschied ist
 * wichtig genug, ihn nicht zu verwischen:
 *
 *   Ein ENTWURF ist nie beim Kunden gewesen. Er darf verschwinden.
 *   Ein VERSENDETES oder beauftragtes Angebot ist hinausgegangen. Der
 *   Kunde hat es. Das wird nicht gelöscht, sondern zurückgezogen — mit
 *   Grund und Zeitpunkt. Sonst behauptet die Datenbank hinterher, es habe
 *   dieses Angebot nie gegeben, während es beim Kunden im Postfach liegt.
 */
r.post('/:id/loeschen', intern, async (req, res, next) => {
  try {
    const id = Number(req.params.id);
    const a = await db.one(
      'SELECT a.*, k.firma FROM angebote a JOIN kunden k ON k.id=a.kunde_id WHERE a.id=$1', [id]);
    if (!a) return res.redirect('/angebote');

    const rechnungen = await db.one(
      'SELECT COUNT(*)::int AS n FROM rechnungen WHERE angebot_id=$1', [id]);
    if (rechnungen.n > 0) {
      res.melde(`Zu diesem Angebot gibt es ${rechnungen.n} Rechnung(en). `
        + 'Ein Angebot mit Rechnungsbezug wird nicht entfernt — sonst hinge die Rechnung in der Luft.', 'warn');
      return res.redirect(`/angebote/${id}`);
    }

    const istEntwurf = a.status === 'Entwurf';
    if (istEntwurf) {
      await db.tx(async (c) => {
        await c.query('DELETE FROM angebot_positionen WHERE angebot_id=$1', [id]);
        await c.query('DELETE FROM angebote WHERE id=$1', [id]);
      });
      await db.query(
        'INSERT INTO aktivitaeten (kunde_id, projekt_id, benutzer_id, text) VALUES ($1,$2,$3,$4)',
        [a.kunde_id, a.projekt_id, req.benutzer.id,
         `Angebotsentwurf ${a.nummer}${a.bauvorhaben ? ' (' + a.bauvorhaben + ')' : ''} gelöscht.`]
      );
      await auth.protokoll(req, 'angebot_geloescht', 'angebot', id,
        { nummer: a.nummer, status: a.status, netto: a.netto });
      res.melde(`Entwurf ${a.nummer} gelöscht. Der Vorgang steht im Änderungsprotokoll.`);
      return res.redirect('/angebote');
    }

    const grund = h.txt(req.body.grund);
    if (!grund) {
      res.melde(`Angebot ${a.nummer} ist bereits ${a.status.toLowerCase()}. `
        + 'Zum Zurückziehen bitte einen Grund angeben.', 'warn');
      return res.redirect(`/angebote/${id}`);
    }

    await db.query(
      `UPDATE angebote SET status='Zurückgezogen', zurueckgezogen_am=now(),
              zurueckgezogen_grund=$1, geaendert_am=now() WHERE id=$2`, [grund, id]);
    await db.query(
      'INSERT INTO aktivitaeten (kunde_id, projekt_id, benutzer_id, text) VALUES ($1,$2,$3,$4)',
      [a.kunde_id, a.projekt_id, req.benutzer.id,
       `Angebot ${a.nummer} zurückgezogen. Grund: ${grund}`]
    );
    await auth.protokoll(req, 'angebot_zurueckgezogen', 'angebot', id, { nummer: a.nummer, grund });
    res.melde(`Angebot ${a.nummer} zurückgezogen. Es bleibt mit Grund und Zeitpunkt in der Akte.`);
    res.redirect(`/angebote/${id}`);
  } catch (e) { next(e); }
});
