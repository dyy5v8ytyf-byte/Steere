...
r.post('/neu', intern, async (req, res, next) => {
  try {
    ...
    const beg = await db.tx(async (c) => {
      /*
       * Eine Begehung IST ein Projekt.
       *
       * Bis 3.6 konnte eine Begehung ohne Projekt entstehen. Das spätere
       * Angebot hing dann an nichts, der Termin an nichts, die Aufgabe an
       * nichts — und im Projektbestand fehlte ein Vorgang, an dem längst
       * gearbeitet wurde. Deshalb: Ist kein Projekt gewählt, wird eines
       * angelegt. Der Anwender muss nicht daran denken.
       */
      let projektId = h.zuZahl(b.projekt_id);
      const kundeId = h.zuZahl(b.kunde_id);
      if (!projektId && kundeId) {
        const p = (await c.query(
          `INSERT INTO projekte (kunde_id, bezeichnung, phase, gewerk, projektmanager_id, letzte_aktivitaet)
           VALUES ($1,$2,$3,$4,$5, now()) RETURNING id`,
          [kundeId, objekt, 'Begehung', h.txt(b.art) === 'DC' ? 'DC-Ladeinfrastruktur' : 'AC-Ladeinfrastruktur',
           h.zuZahl(b.projektmanager_id) || null]
        )).rows[0];
        projektId = p.id;
      }
      ...
      const x = (await c.query(
        ...
        [nummer, kundeId, projektId, objekt, h.txt(b.adresse),
      ...
      if (projektId) {
        await c.query('UPDATE projekte SET aus_begehung_id = COALESCE(aus_begehung_id, $1) WHERE id = $2',
          [x.id, projektId]);
        // Der Begehungstermin gehoert in die Terminliste, sonst steht er
        // nur in der Begehung und taucht in keiner Wochenplanung auf.
        if (h.txt(b.termin_am)) {
          await c.query(
            `INSERT INTO termine (kunde_id, projekt_id, begehung_id, typ, datum, ort, thema, status)
             VALUES ($1,$2,$3,'Begehung',$4::date,$5,$6,'geplant')`,
            [kundeId, projektId, x.id, h.txt(b.termin_am),
             [h.txt(b.adresse), h.txt(b.plz), h.txt(b.ort)].filter(Boolean).join(', ') || null,
             `Begehung ${nummer} — ${objekt}`]
          );
        }
      }
...
/**
 * Nachtraeglich ein Projekt zu einer Begehung anlegen.
 *
 * Fuer alles, was vor 3.6 entstanden ist: Begehungen ohne Projekt. Der Knopf
 * legt das Projekt an, haengt die Begehung daran und zieht vorhandene
 * Angebote, Termine und Aufgaben desselben Kunden mit hinein, sofern sie
 * noch an keinem Projekt haengen. Vorhandene Zuordnungen werden nicht
 * angefasst — lieber ein Vorgang zu wenig eingesammelt als einer, der
 * plötzlich am falschen Projekt haengt.
 */
r.post('/:id/projekt', intern, async (req, res, next) => {
  try {
    const id = Number(req.params.id);
    const beg = await db.one(
      'SELECT b.*, k.firma FROM begehungen b JOIN kunden k ON k.id=b.kunde_id WHERE b.id=$1', [id]);
    if (!beg) return res.redirect('/begehungen');

    if (beg.projekt_id) {
      res.melde('Diese Begehung hängt bereits an einem Projekt.', 'warn');
      return res.redirect(`/begehungen/${id}`);
    }

    const ergebnis = await db.tx(async (c) => {
      const p = (await c.query(
        `INSERT INTO projekte (kunde_id, bezeichnung, phase, gewerk, aus_begehung_id,
                               projektmanager_id, letzte_aktivitaet)
         VALUES ($1,$2,$3,$4,$5,$6, now()) RETURNING *`,
        [beg.kunde_id, beg.objekt || `Begehung ${beg.nummer}`, 'Begehung',
         beg.art === 'DC' ? 'DC-Ladeinfrastruktur' : 'AC-Ladeinfrastruktur',
         beg.id, h.zuZahl(req.body.projektmanager_id) || null]
      )).rows[0];

      await c.query('UPDATE begehungen SET projekt_id=$1, geaendert_am=now() WHERE id=$2', [p.id, id]);

      const ang = await c.query(
        'UPDATE angebote SET projekt_id=$1 WHERE begehung_id=$2 AND projekt_id IS NULL RETURNING id', [p.id, id]);
      const ter = await c.query(
        'UPDATE termine SET projekt_id=$1 WHERE begehung_id=$2 AND projekt_id IS NULL RETURNING id', [p.id, id]);
      const auf = await c.query(
        'UPDATE aufgaben SET projekt_id=$1 WHERE begehung_id=$2 AND projekt_id IS NULL RETURNING id', [p.id, id]);

      // Der Begehungstermin selbst, falls er noch in keiner Terminliste steht.
      let terminNeu = 0;
      if (beg.termin_am) {
        const vorhanden = (await c.query('SELECT id FROM termine WHERE begehung_id=$1 LIMIT 1', [id])).rows[0];
        if (!vorhanden) {
          await c.query(
            `INSERT INTO termine (kunde_id, projekt_id, begehung_id, typ, datum, ort, thema, status)
             VALUES ($1,$2,$3,'Begehung',$4::date,$5,$6,'geplant')`,
            [beg.kunde_id, p.id, id, beg.termin_am,
             [beg.adresse, beg.plz, beg.ort].filter(Boolean).join(', ') || null,
             `Begehung ${beg.nummer} — ${beg.objekt || ''}`.trim()]
          );
          terminNeu = 1;
        }
      }
      ...
      await c.query(
        'INSERT INTO aktivitaeten (kunde_id, projekt_id, benutzer_id, text) VALUES ($1,$2,$3,$4)',
        [beg.kunde_id, p.id, req.benutzer.id,
         `Projekt aus Begehung ${beg.nummer} angelegt.`]
      );
      return { projekt: p, angebote: ang.rowCount, termine: ter.rowCount + terminNeu, aufgaben: auf.rowCount };
    });

    await auth.protokoll(req, 'projekt_aus_begehung', 'projekt', ergebnis.projekt.id, { begehung: beg.nummer });
    res.melde(`Projekt „${ergebnis.projekt.bezeichnung}" angelegt und verknüpft — `
      + `${ergebnis.angebote} Angebot(e), ${ergebnis.termine} Termin(e), ${ergebnis.aufgaben} Aufgabe(n).`);
    res.redirect(`/begehungen/${id}`);
  } catch (e) { next(e); }
});
