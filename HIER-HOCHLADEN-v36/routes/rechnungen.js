...
r.get('/', intern, async (req, res, next) => {
  try {
    ...
    let wo = 'TRUE';
    if (filter === 'offen') wo = "rg.festgeschrieben AND rg.bezahlt_am IS NULL AND rg.ausgeglichen_am IS NULL AND rg.art <> 'storno'";
    else if (filter === 'ueberfaellig') wo = "rg.festgeschrieben AND rg.bezahlt_am IS NULL AND rg.ausgeglichen_am IS NULL AND rg.art <> 'storno' AND rg.faellig_am < CURRENT_DATE";
    ...
    const summen = await db.one(`
      SELECT COALESCE(SUM(brutto) FILTER (WHERE festgeschrieben AND bezahlt_am IS NULL AND ausgeglichen_am IS NULL AND art<>'storno'),0) AS offen,
             COALESCE(SUM(brutto) FILTER (WHERE festgeschrieben AND bezahlt_am IS NULL AND ausgeglichen_am IS NULL AND art<>'storno' AND faellig_am < CURRENT_DATE),0) AS ueberfaellig,
...
r.post('/:id/stornieren', intern, async (req, res, next) => {
  try {
    ...
    const storno = await db.tx(async (c) => {
      ...
      /*
       * Die Gutschrift gleicht die Rechnung aus.
       *
       * Vorher blieb eine stornierte Rechnung unter den offenen Posten
       * stehen und wurde weiter angemahnt — fachlich falsch, denn sie ist
       * durch die Gutschrift erledigt. Ein Zahlungseingang wird trotzdem
       * NICHT vorgetäuscht: `bezahlt_am` bleibt leer, ausgeglichen wird in
       * einem eigenen Feld vermerkt. Wer die Zahlen prüft, sieht dann den
       * Unterschied zwischen „bezahlt" und „storniert und ausgeglichen".
       */
      await c.query(
        'UPDATE rechnungen SET storniert_durch=$1, ausgeglichen_am=now() WHERE id=$2', [s.id, rg.id]);
      await c.query('UPDATE rechnungen SET ausgeglichen_am=now() WHERE id=$1', [s.id]);
