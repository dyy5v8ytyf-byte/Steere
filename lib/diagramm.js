...
function abweichung({ titel, punkte, hoehe = 200, id = 'd2' }) {
  ...
  s += `<text x="${links - 8}" y="${oben + 10}" class="dia-achse" text-anchor="end">+${kurz(max)}</text>`;
  s += `<text x="${links - 8}" y="${oben + ih}" class="dia-achse" text-anchor="end">−${kurz(max)}</text>`;

  punkte.forEach((p, i) => {
    const w = p.wert || 0;
    const x = links + proSpalte * i + (proSpalte - b) / 2;
    const laenge = (Math.abs(w) / max) * (ih / 2);
    const ueber = w >= 0;
...
module.exports = { sollIst, abweichung, ring, messlatte, kurz, esc };
