 ...
 */
const GRUND_BEREICHE = ['offline_access', 'User.Read', 'Calendars.ReadWrite', 'Mail.Send'];

/**
 * Planner und Teams verlangen mehr, als der Rest der Anwendung braucht:
 *
 *   Tasks.ReadWrite        Planner-Aufgaben lesen und schreiben
 *   Group.Read.All         die Pläne der eigenen Gruppen finden
 *   Team.ReadBasic.All     die eigenen Teams auflisten
 *   ChannelMessage.Send    in einen Kanal schreiben
 *
 * Group.Read.All stuft Microsoft in den meisten Mandanten als
 * zustimmungspflichtig ein — ohne Administrator geht es dort nicht. Deshalb
 * sind diese Berechtigungen abschaltbar: Wer M365_PLANNER=aus setzt (oder
 * die Variable weglässt und die Zustimmung nicht bekommt), verbindet sich
 * mit dem kleinen Satz und behält Kalender und Mail. Ein einziger großer
 * Berechtigungsblock, der bei der Zustimmung scheitert, würde sonst auch
 * das Funktionierende mitreißen.
 */
const ERWEITERTE_BEREICHE = ['Tasks.ReadWrite', 'Group.Read.All', 'Team.ReadBasic.All', 'ChannelMessage.Send'];

function plannerGewuenscht() {
  return process.env.M365_PLANNER !== 'aus';
}

const BEREICHE = plannerGewuenscht()
  ? GRUND_BEREICHE.concat(ERWEITERTE_BEREICHE)
  : GRUND_BEREICHE;
...
module.exports = {
  BEREICHE, GRUND_BEREICHE, ERWEITERTE_BEREICHE, plannerGewuenscht,
  eingerichtet, einstellungen, rueckgabeAdresse,
